...
| `WP_REDIS_PERSISTENT`                | `false`     | Whether to reuse the connection across requests instead of reconnecting on every one (ignored when sharding) |
| `WP_REDIS_PERSISTENT_ID`             |             | The identifier persistent connections are pooled by, defaults to `wp-db{database}` |

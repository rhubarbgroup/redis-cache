...
| `WP_REDIS_PERSISTENT`                | `false`     | Whether to reuse the connection across requests instead of reconnecting on every one (ignored when sharding). A non-empty string also names the pool, which defaults to `wp-db{database}` |
